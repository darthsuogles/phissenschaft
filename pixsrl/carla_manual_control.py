...
class CarlaGame(object):
    ...
    def _initialize_game(self):
        ...
        if self._city_name is not None:
            ...
            self._map_view = self._map.get_map(MINI_WINDOW_HEIGHT)

            extra_width = int((WINDOW_HEIGHT / float(self._map_shape[0])) *
                              self._map_shape[1])
            resolution = (WINDOW_WIDTH + extra_width, WINDOW_HEIGHT)
        else:
            extra_width = 0
        ...
        WINDOW_WIDTH = display_info.current_w - extra_width
    ...
    def _on_render(self):
        ...
        mini_image_y = WINDOW_HEIGHT - MINI_WINDOW_HEIGHT
        ...
        if self._lidar_measurement is not None:
            ...
            #draw lidar
            lidar_img_size = (MINI_WINDOW_WIDTH, MINI_WINDOW_WIDTH, 3)
            lidar_img = np.zeros(lidar_img_size)
            lidar_img[tuple(lidar_data.T)] = (255, 255, 255)
            surface = pygame.surfarray.make_surface(lidar_img)
            #self._display.blit(surface, (10, 10))
            self._display.blit(surface,
                               (3 * gap_x + 2 * MINI_WINDOW_WIDTH, mini_image_y))
        ...
        if self._map_view is not None:
            ...
            # TODO: make sure the map view position is configured so that
            #       it is localized to its allocated position
            new_window_width = \
                (float(MINI_WINDOW_HEIGHT) / float(self._map_shape[0])) * \
            ...
            self._display.blit(surface,
                               (4 * gap_x + 3 * MINI_WINDOW_WIDTH, mini_image_y))
