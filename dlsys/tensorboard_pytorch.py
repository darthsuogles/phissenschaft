...
for n_iter in range(100):
    ...
    if n_iter % 10 == 0:
        ...
        for i in range(x.size(0)):
            ...
            dummy_audio[i] = np.cos(
                freqs[n_iter // 10] * np.pi * float(i) / float(sample_rate))
        writer.add_audio(
            'myAudio', dummy_audio, n_iter, sample_rate=sample_rate)
...
with SummaryWriter(comment="FacilePyTorch") as writer:
    writer.add_graph(Facile(), (torch.zeros(1, 3),), True)
