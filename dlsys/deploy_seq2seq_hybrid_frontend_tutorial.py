...
class GreedySearchDecoder(torch.jit.ScriptModule):
    ...
    def forward(self, input_seq: torch.Tensor, input_length: torch.Tensor,
                max_length: int):
...
loadFilename = dataset_root / "4000_checkpoint.tar"
...
raw_sentence = "able was I ere I saw"
sentence = normalizeString(raw_sentence)
indexes_batch = [indexesFromSentence(voc, sentence)]
# Create lengths tensor
lengths = torch.tensor([len(indexes) for indexes in indexes_batch])
# Transpose dimensions of batch to match models' expectations
...
encoder.forward(input_batch, lengths)

with SummaryWriter(comment="Seq2Seq") as writer:
    writer.add_graph(encoder, (input_batch, lengths))
