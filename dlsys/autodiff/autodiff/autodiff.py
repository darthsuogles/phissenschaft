...
class Node(object):
    ...
    def __mul__(self, other):
        """Multiplying two nodes and return a new node"""
        if isinstance(other, Node):
...
class MulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        A, B = node.inputs
        return [mul_op(output_grad, B), 
                mul_op(output_grad, A)]
...
class Executor:
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            ...
            curr_inputs = [node_to_val_map[in_node] for in_node in node.inputs]
            node_to_val_map[node] = node.op.compute(node, curr_inputs)
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        curr_out_grads = node_to_output_grads_list[node]
        grad = sum_node_list(curr_out_grads)
        in_grads = node.op.gradient(node, grad)
        node_to_output_grad[node] = grad
        if not in_grads: continue
