...
def squash(vectors, axis=-1):
    ...
    scale = s_squared_norm / (
        1 + s_squared_norm) / K.sqrt(s_squared_norm + K.epsilon())
    return scale * vectors


class CapsuleLayer(Lyr.Layer):
    """
    ...
    from scalar to vector. So its input shape = [None, input_num_capsule, input_dim_vector] and
    ...
    def build(self, input_shape):
        ...
        # Coupling coefficient. The redundant dimensions are just to
        # facilitate subsequent matrix calculation.
    ...
    def call(self, inputs, training=None):
        ...
        inputs_hat = tf.scan(
            lambda ac, x: K.batch_dot(x, self.W, [3, 2]),
            elems=inputs_tiled,
            initializer=K.zeros(
        ...
        for i in range(self.num_routing):
            c = tf.nn.softmax(
                self.bias, dim=2)  # dim=2 is the num_capsule dimension
...
def PrimaryCap(inputs, dim_vector, n_channels, kernel_size, strides, padding):
    ...
    outputs = Lyr.Reshape(
        target_shape=[-1, dim_vector], name='primarycap_reshape')(output)
    return Lyr.Lambda(squash, name='primarycap_squash')(outputs)
...
def CapsNet(input_shape, n_class, num_routing):
    ...
    # Decoder network.
    y = Lyr.Input(shape=(n_class, ))
    masked = Mask()(
        [digitcaps,
         y])  # The true label is used to mask the output of capsule layer.
    x_recon = Lyr.Dense(512, activation='relu')(masked)
    x_recon = Lyr.Dense(1024, activation='relu')(x_recon)
    x_recon = Lyr.Dense(np.prod(input_shape), activation='sigmoid')(x_recon)
    x_recon = Lyr.Reshape(
        target_shape=input_shape, name='out_recon')(x_recon)
